...
static void set_video_clock( unsigned long pll_reg, unsigned long clk_reg, unsigned long vid_div_reg, unsigned long xd )
{
    unsigned int tmp;
    

    tmp = READ_MPEG_REG(HHI_VID_CLK_CNTL) & (1 << 20);
    ...
    WRITE_MPEG_REG( HHI_VID_PLL_CNTL2,       0x40003 );  
    WRITE_MPEG_REG( HHI_VID_PLL_CNTL3,       0x40e8 );  
    
    WRITE_MPEG_REG( HHI_VID_CLK_DIV, (READ_MPEG_REG(HHI_VID_CLK_DIV) & ~(0xFF << 0)) | (xd-1) );   // setup the XD divider value
    // delay 5uS
	udelay(5);
	
	WRITE_MPEG_REG( HHI_VID_CLK_CNTL, (clk_reg & (~(1 << 20))) ); 	
    WRITE_MPEG_REG( HHI_VID_CLK_CNTL, READ_MPEG_REG(HHI_VID_CLK_CNTL) |  (1 << 19) );     //enable clk_div0 
    WRITE_MPEG_REG( HHI_VID_CLK_CNTL, READ_MPEG_REG(HHI_VID_CLK_CNTL) | tmp |1);     //change back clk_div1 
    // delay 2uS
	udelay(2);
}
...
static inline void _init_tvenc(tcon_conf_t *pConf)
{
	...
	unsigned pll_reg, clk_reg, xd;
	pll_reg = (pConf->pll_ctrl) & 0xffff;			
	xd = (pConf->clk_ctrl) & 0xf;
	clk_reg = (pConf->clk_ctrl) & 0xfff0;			
	set_video_clock(pll_reg, clk_reg, 0x3143, xd);

    WRITE_MPEG_REG(ENCP_VIDEO_FILT_CTRL,    0x1000);
    
    WRITE_MPEG_REG(VENC_DVI_SETTING,        0x11);
  
		WRITE_MPEG_REG(VENC_C656_CTRL, READ_MPEG_REG(VENC_C656_CTRL) | 0x8000);    //HDMI output "LCD RGB(6*3 bits)"
		//WRITE_MPEG_REG(VPP_POSTPROC_MISC, READ_MPEG_REG(VPP_POSTPROC_MISC)|0x80); // enable vpp async fifo interface        
    
        WRITE_MPEG_REG(ENCI_SYNC_HOFFST, 0x1b); // add by karlson to delete verical stripe

    WRITE_MPEG_REG(ENCP_VIDEO_MODE,         0x0040);
        WRITE_MPEG_REG(ENCP_VIDEO_MODE_ADV, 0x18);
        WRITE_MPEG_REG(ENCP_VIDEO_MODE_ADV,
        READ_MPEG_REG(ENCP_VIDEO_MODE_ADV) | 0x0400);
        
		WRITE_MPEG_REG(ENCP_VIDEO_YFP1_HTIME, 64);  //520);
		WRITE_MPEG_REG(ENCP_VIDEO_YFP2_HTIME, 1056);  //3080);

    WRITE_MPEG_REG(ENCP_VIDEO_MAX_PXCNT,    pConf->max_width - 1);
    WRITE_MPEG_REG(ENCP_VIDEO_MAX_LNCNT,    pConf->max_height - 1);

        WRITE_MPEG_REG(ENCP_VIDEO_HSPULS_BEGIN, 3245);  //99);  //3220);karlson add,no same to 861B
    ...
    WRITE_MPEG_REG(ENCP_VIDEO_VSPULS_BEGIN, 0);
        WRITE_MPEG_REG(ENCP_VIDEO_VSPULS_END, 3079);    //0; huanghonglin for bug#8549 2005-9-19
    WRITE_MPEG_REG(ENCP_VIDEO_VSPULS_BLINE, 0);
    WRITE_MPEG_REG(ENCP_VIDEO_VSPULS_ELINE, 4);

		WRITE_MPEG_REG(ENCP_VIDEO_HAVON_BEGIN,  64);
		WRITE_MPEG_REG(ENCP_VIDEO_HAVON_END,    pConf->width - 1 + 64);
