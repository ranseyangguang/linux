...
int sd_identify_process(SD_MMC_Card_Info_t *sd_mmc_info)
{
	...
	unsigned char write_reg_data, read_reg_data;
	...
	if(SD_WORK_MODE == CARD_HW_MODE)
	{
		...
		{
			config_reg->cmd_clk_divide = get_mpeg_clk()/1000000 / (2*SD_MMC_TRANSFER_HIGHSPEED_CLK) -1;
...
int sd_mmc_switch_function(SD_MMC_Card_Info_t *sd_mmc_info)
{
	...
	printk("set sd_mmc config_reg->cmd_clk_divide %d, CLK %dM\n", 
		config_reg->cmd_clk_divide, get_mpeg_clk()/2000000/(config_reg->cmd_clk_divide + 1));
