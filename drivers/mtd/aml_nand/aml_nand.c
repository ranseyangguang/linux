...
static int aml_nand_add_partition(struct aml_nand_chip *aml_chip)
{
	...
	dev_dbg (aml_chip->device, "nand add partition (nr=%d)\n", nr);
...
static bool dma_ok(void *ptr)
{
	unsigned long addr = (unsigned long) ptr;
	if (addr < PAGE_OFFSET)
		return false;
	if (addr & (dma_get_cache_alignment() - 1))
		return false;
	return true;
}
...
static int aml_platform_dma_write(struct aml_nand_chip *aml_chip, unsigned char *buf, int len, unsigned bch_mode)
{
	...
	dma_addr_t user_data_dma_addr;
	bool bounce;
	void *data_dma_buf;

	bounce = !dma_ok(buf);
	if (bounce) {
		data_dma_buf = aml_chip->aml_nand_data_buf;
		memcpy (data_dma_buf, buf, len);
	} else {
		data_dma_buf = buf;
	}
		...
		if (aml_chip->valid_chip[i]) {
			...
			for (j=0; j<internal_chipnr; j++) {
				...
				if (aml_chip->plane_num == 2) {
					...
					if (stat < 0) {
						mtd->ecc_stats.failed++;
						aml_nand_debug("read oob ecc plane0 failed at blk %d chip: %d\n", (page_addr >> pages_per_blk_shift), i);
//						error = -EIO;goto exit;
					}
					else
						mtd->ecc_stats.corrected += stat;
					...
					if (nand_read_size > 0) {
						...
						if (stat < 0) {
							mtd->ecc_stats.failed++;
							aml_nand_debug("read oob ecc plane1 failed at blk %d\n", (page_addr >> pages_per_blk_shift));
//							error = -EIO;goto exit;
						}
						else
							mtd->ecc_stats.corrected += stat;
...
{ 
	...
	chip->pagebuf = -1;
	//return 0;
//	dev_dbg (&mtd->dev, "aml_nand_block_bad");
...
printk ("aml_chip, pagesz = %d, blocksz = %d, oob_size = %d\n", aml_type->pagesize, aml_type->erasesize, aml_type->oobsize);
...
int aml_nand_init(struct aml_nand_chip *aml_chip)
{
	...
	spin_lock_init(&aml_chip->controller.lock);
	init_waitqueue_head(&aml_chip->controller.wq);
	chip->controller = &aml_chip->controller;
...
//	aml_chip->aml_nand_data_buf=kzalloc((mtd->writesize + mtd->oobsize+64),GFP_KERNEL);
	aml_chip->aml_nand_data_buf=kzalloc(8704,GFP_KERNEL); //mtd->writesize not known yet
