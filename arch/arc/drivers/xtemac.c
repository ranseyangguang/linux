...
void XEMAC_mdio_write(volatile struct xtemac_ptr *, unsigned int, unsigned int,
		      unsigned int);
...
static irqreturn_t xtemac_emac_intr(int irq, void *dev_instance)
{
	...
	if (status & INTR_STATUS_MDIO_BIT_POS) {
		...
		arc_write_uncached_32(&xtemac->bridge.intrClrReg,
				      INTR_STATUS_MDIO_BIT_POS);
	}
	...
	if (status & INTR_STATUS_RX_ERROR_BIT_POS) {
		...
		arc_write_uncached_32(&xtemac->bridge.intrClrReg,
				      INTR_STATUS_RX_ERROR_BIT_POS);
		arc_write_uncached_32(&xtemac->bridgeExtn.intrEnableReg,
				      ENABLE_ALL_INTR &
				      (~ENABLE_RX_OVERRUN_INTR));
	...
	if (status & INTR_STATUS_DMA_RX_COMPLETE_BIT_POS) {
		...
		flush_and_inv_dcache_range(priv->rx_buff,
					   priv->rx_buff + priv->rx_len);
	...
	if (status & INTR_STATUS_RCV_INTR_BIT_POS) {
		...
		arc_write_uncached_32(&xtemac->bridge.intrClrReg,
				      INTR_STATUS_RCV_INTR_BIT_POS);
		arc_write_uncached_32(&xtemac->bridgeExtn.intrEnableReg,
				      ENABLE_ALL_INTR & (~ENABLE_PKT_RECV_INTR)
				      & (~ENABLE_RX_OVERRUN_INTR));

		priv->rx_len =
		    arc_read_uncached_32(&xtemac->bridgeExtn.sizeReg);
		arc_write_uncached_32(&xtemac->bridge.dmaRxAddrReg,
				      priv->rx_buff);
		inv_dcache_range(priv->rx_buff, priv->rx_buff + priv->rx_len);
		arc_write_uncached_32(&xtemac->bridge.dmaRxCmdReg,
				      (priv->rx_len << 8 | DMA_READ_COMMAND));
	}
	...
	if (status & INTR_STATUS_RX_OVRN_INTR_BIT_POS) {
		...
		arc_write_uncached_32(&xtemac->bridge.dmaRxClrReg, 1);
		arc_write_uncached_32(&xtemac->bridge.intrClrReg,
				      INTR_STATUS_RX_OVRN_INTR_BIT_POS);
		arc_write_uncached_32(&xtemac->config.rxCtrl1Reg, 0);
		arc_write_uncached_32(&xtemac->bridge.txFifoRxFifoRstReg,
				      RESET_TX_RX_FIFO);
...
int xtemac_open(struct net_device *dev)
{
	...
	arc_write_uncached_32(&xtemac->config.macModeConfigReg,
			      SET_100MBPS_MODE);

	/* Probe the Phy to see what speed is negotiated. */
	XEMAC_mdio_read(xtemac, BSP_XEMAC1_PHY_ID, MV_88E1111_STATUS2_REG,
			&speed);
	printk("Phy setup\n");
	...
	XEMAC_mdio_write(xtemac, BSP_XEMAC1_PHY_ID, MV_88E1111_CTRL_REG,
			 MV_88E1111_CTRL_RESET);
	do {
		XEMAC_mdio_read(xtemac, BSP_XEMAC1_PHY_ID, MV_88E1111_CTRL_REG,
				&temp);
	...
	temp =
	    MV_88E1111_AUTONEG_ADV_100BTX_FULL | MV_88E1111_AUTONEG_ADV_100BTX |
	    AUTONEG_ADV_IEEE_8023;

	XEMAC_mdio_write(xtemac, BSP_XEMAC1_PHY_ID, MV_88E1111_AUTONEG_ADV_REG,
			 temp);
	...
	XEMAC_mdio_write(xtemac, BSP_XEMAC1_PHY_ID, MV_88E1111_CTRL_REG,
			 (MV_88E1111_CTRL_AUTONEG |
			  MV_88E1111_CTRL_RESTART_AUTO));

	do {
		XEMAC_mdio_read(xtemac, BSP_XEMAC1_PHY_ID,
				MV_88E1111_STATUS_REG, &temp);
	...
	arc_write_uncached_32(&xtemac->bridgeExtn.intrEnableReg,
			      ENABLE_ALL_INTR & (~ENABLE_RX_OVERRUN_INTR));

	return 0;
}

/* XTEMAC close routine */
int xtemac_stop(struct net_device *dev)
{
	return 0;
}

/* XTEMAC ioctl commands */
int xtemac_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	printk("ioctl called\n");
...
int xtemac_tx(struct sk_buff *skb, struct net_device *dev)
{
...
/*        printk("Dropping due to not enough room in the TXFIFO\n"); */
	...
	arc_write_uncached_32(&xtemac->bridge.dmaTxCmdReg,
			      (skb->
			       len << 8 | DMA_WRITE_COMMAND |
			       DMA_START_OF_PACKET | DMA_END_OF_PACKET));
...
void xtemac_tx_timeout(struct net_device *dev)
{
...
void xtemac_set_multicast_list(struct net_device *dev)
{
	printk("set multicast list called\n");
	return;
}

int xtemac_set_address(struct net_device *dev, void *p)
{
...
struct net_device_stats *xtemac_stats(struct net_device *dev)
{
...
void XEMAC_mdio_write(
	...
	arc_write_uncached_32((&dev_ptr->bridgeExtn.macMDIOReg + reg_index),
			      data);
	...
	while (!(value & INTR_STATUS_MDIO_BIT_POS)) {
		value =
		    arc_read_uncached_32(&dev_ptr->bridgeExtn.intrRawStsReg);
	}

	arc_write_uncached_32(&dev_ptr->bridge.intrClrReg,
			      INTR_STATUS_MDIO_BIT_POS);
			     ...
			     volatile unsigned int *data_ptr)
{
	...
	arc_write_uncached_32(&dev_ptr->bridgeExtn.phyAddrReg, phy_addr);
	*data_ptr =
	    arc_read_uncached_32(&dev_ptr->bridgeExtn.macMDIOReg + reg_index);
...
int __init xtemac_module_init(void)
{
...
void __exit xtemac_module_cleanup(void)
{
	return;
