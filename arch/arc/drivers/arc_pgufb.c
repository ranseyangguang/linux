...
static int arc_pgu_ioctl(struct fb_info *info, unsigned int cmd,
			 unsigned long arg);
...
static void stop_arcpgu(void)
{
	...
	while ((fb_devdata.regs->statctrl & STATCTRL_DISP_BUSY_MASK)
	       && (fb_devdata.regs->statctrl & STATCTRL_BU_BUSY_MASK)) ;
...
static int calculate_yuv_offsets(struct yuv_info *info)
{
	...
	tmpsize =
	    info->num_buffers * ((info->height * info->width) +
	...
	info->u_offset =
	    BUF_ALIGN(((info->width * info->height) + info->alignment),
		      info->alignment);
	info->v_offset =
	    BUF_ALIGN((info->u_offset + tmpsize + info->alignment),
		      info->alignment);
	...
	fb_devdata.yuv_base[0] =
	    BUF_ALIGN((info->start_offset + info->alignment), info->alignment);
	for (i = 1; i < info->num_buffers; i++) {
		fb_devdata.yuv_base[i] =
		    BUF_ALIGN((fb_devdata.yuv_base[i - 1] + tmpsize +
...
static void blank_yuv(struct yuv_info *info)
{
	...
	for (i = 0; i < info->num_buffers; i++) {
		VOLATILE_MEMSET(((void *)fb_devdata.fb_virt_start) +
				fb_devdata.yuv_base[i], 0,
				(info->width * info->height));
		VOLATILE_MEMSET(((void *)fb_devdata.fb_virt_start) +
				fb_devdata.yuv_base[i] +
				(info->width * info->height), 0x80,
				(info->width * info->height) / 2);
	}
}
...
static void arc_pgu_setdims(struct yuv_info *info)
{
	...
	width =
	    (info->width >
	     fb_devdata.display->xres) ? fb_devdata.display->xres -
	    1 : info->width - 1;
...
static int set_yuv_pointer(void)
{
	...
	fb_devdata.regs->frm_start =
	    (unsigned int)((void *)fb_devdata.yuv.virt_base) +
...
static int set_rgb_pointer(void)
{
	...
	fb_devdata.regs->frm_start = (unsigned int)fb_devdata.fb_virt_start
	    +
	    (current_par.rgb_bufno * fb_devdata.display->xres *
	     fb_devdata.display->yres * (fb_devdata.display->bpp / 8));

	fb_devdata.regs->stride =
	    fb_devdata.display->xres * (fb_devdata.display->bpp / 8);

	return 0;
}

static void set_color_bitfields(struct arc_pgu_devdata *fb_devdata,
...
static int arc_pgu_setmode(void)
{
	...
	fb_devdata.regs->disp_dim =
	    ENCODE_PGU_DIMS((fb_devdata.display->xres - 1),
			    (fb_devdata.display->yres - 1));
	...
	if ((fb_devdata.regs->hsync_cfg != DMT_75_HSYNC)
	    || (fb_devdata.regs->vsync_cfg != DMT_75_VSYNC)) {
		printk(KERN_INFO
		       "arc_pgu: old PGU version (0x%08lx 0x%08lx) ... disabling VCLK control\n",
	...
	printk(KERN_INFO
	       "arc_pgu_setmode: fbbase 0x%lx, ybase 0x%lx, ubase 0x%lx, vbase 0x%lx\n",
	       fb_devdata.fb_virt_start, fb_devdata.regs->frm_start,
	       fb_devdata.regs->cru_frm_st, fb_devdata.regs->crv_frm_st);

	fb_devdata.regs->ol_frm_st = fb_devdata.fb_virt_start;
	fb_devdata.regs->ol_stride = fb_devdata.display->xres * 2;	/* Always */
	fb_devdata.regs->ol_scn_line =
	    ENCODE_PGU_DIMS((0), (fb_devdata.display->yres - 1));
	fb_devdata.regs->ol_col_key = 0;

	set_rgb_pointer();
	fb_devdata.regs->statctrl |=
	    (STATCTRL_CONT_B_MASK | current_par.main_mode | current_par.
	     overlay_mode | fb_devdata.display->control);
	if (fb_devdata.display->bl_active_high)
		fb_devdata.regs->statctrl |= STATCTRL_BACKLIGHT_MASK;
	else
		fb_devdata.regs->statctrl &= ~STATCTRL_BACKLIGHT_MASK;
			 ...
			 unsigned long arg)
{
	...
	case ARCPGUFB_GETYUV:
		return copy_to_user((void *)arg, (void *)&(fb_devdata.yuv),
				    sizeof(struct yuv_info)) ? -EFAULT : 0;
	...
	case ARCPGUFB_SETYUV:
		result =
		    copy_from_user(&tmpyuv, (void *)arg,
				   sizeof(struct yuv_info)) ? -EFAULT : 0;
		if (!result) {
			if (tmpyuv.alignment % BASIC_ALIGNMENT)
				return -EFAULT;
			down(&pgu_sem);
			tmplong = fb_devdata.regs->statctrl;
			stop_arcpgu();
			memcpy(&fb_devdata.yuv, &tmpyuv,
			       sizeof(struct yuv_info));
			arc_pgu_setdims(&(fb_devdata.yuv));
			set_yuv_pointer();
	...
	case ARCPGUFB_FILLYUV:
		result =
		    copy_from_user(&tmpyuv, (void *)arg, sizeof(unsigned long));
		if (!result) {
			calculate_yuv_offsets(&tmpyuv);
			result =
			    copy_to_user((void *)arg, (void *)&(tmpyuv),
					 sizeof(struct yuv_info)) ? -EFAULT : 0;
		}
		return result;
	...
	case ARCPGUFB_SET_YUV_RES:
		if (copy_from_user
		    (&(tmpyuv), (void *)arg, sizeof(struct yuv_info)))
			return -EFAULT;
		fb_devdata.yuv.width = tmpyuv.width;
		fb_devdata.yuv.height = tmpyuv.height;

		if (fb_devdata.yuv.width & 0xf)
			fb_devdata.yuv.width =
			    ((fb_devdata.yuv.width + 0xf) & ~0xf);
		if (fb_devdata.yuv.height & 0xf)
			fb_devdata.yuv.height =
			    ((fb_devdata.yuv.height + 0xf) & ~0xf);

		fb_devdata.yuv.y_stride = tmpyuv.width;
		fb_devdata.yuv.u_stride = fb_devdata.yuv.v_stride =
		    tmpyuv.width / 2;
		if (calculate_yuv_offsets(&(fb_devdata.yuv)))
			return -EFAULT;
	...
	case ARCPGUFB_RESET:
		memcpy(&fb_devdata.yuv, &fb_devdata.default_yuv,
		       sizeof(struct yuv_info));
		fb_devdata.yuv.displayed_buffer = 0;
		down(&pgu_sem);
		stop_arcpgu();
	...
	case ARCPGUFB_SWITCH_YUV:
		...
		if (!copy_from_user
		    (&result, (void *)arg, sizeof(unsigned long)))
	...
	case ARCPGUFB_GET_PGU_STATE:
		...
		up(&pgu_sem);
		return copy_to_user((void *)arg, (void *)&temp_pgu,
				    sizeof(arc_pgu)) ? -EFAULT : 0;

	case ARCPGUFB_SET_PGU_STATE:
		down(&pgu_sem);
		stop_arcpgu();
		result =
		    copy_from_user((void *)&temp_pgu, (void *)arg,
				   sizeof(arc_pgu)) ? -EFAULT : 0;
		if (!result) {
	...
	case ARCPGUFB_GET_FB_INFO:
		return copy_to_user((void *)arg, &fb_devdata,
				    sizeof(struct arc_pgu_devdata)) ? -EFAULT :
		    0;

	case ARCPGUFB_GET_MAIN_MODE:
		result = fb_devdata.regs->statctrl & STATCTRL_PIX_FMT_MASK;
		return copy_to_user((void *)arg, (void *)result,
				    sizeof(unsigned int)) ? -EFAULT : 0;

	case ARCPGUFB_SET_MAIN_MODE:
		if (!copy_from_user
		    ((void *)&result, (void *)arg, sizeof(unsigned long))) {
			down(&pgu_sem);
	...
	case ARCPGUFB_SET_RGB_BUF:
		if (!copy_from_user
		    (&tmplong, (void *)arg, sizeof(unsigned long))) {
	...
	case ARCPGUFB_GET_RGB_BUF:
		return copy_to_user((void *)arg, (void *)&current_par.rgb_bufno,
				    sizeof(unsigned long)) ? -EFAULT : 0;

	case ARCPGUFB_GET_FB_MAIN:
		return copy_to_user((void *)arg,
				    (void *)&current_par.main_is_fb,
				    sizeof(unsigned long)) ? -EFAULT : 0;
	...
	case ARCPGUFB_GET_OL_COLKEY:
		return copy_to_user((void *)arg,
				    (const void *)&fb_devdata.regs->ol_col_key,
				    sizeof(unsigned long)) ? -EFAULT : 0;

	case ARCPGUFB_SET_OL_COLKEY:
		result =
		    copy_from_user((void *)&(tmplong), (void *)arg,
	...
	case ARCPGUFB_GET_OL_START:
		tmplong =
		    (fb_devdata.regs->
		     ol_scn_line & DISP_DIM_REG_PGU_X_RES_MASK);
		return copy_to_user((void *)arg, &tmplong,
				    sizeof(unsigned long)) ? -EFAULT : 0;

	case ARCPGUFB_SET_OL_START:
		if (!copy_from_user
		    ((void *)&tmplong, (void *)arg, sizeof(unsigned int))) {
			fb_devdata.regs->ol_scn_line &=
			    ~DISP_DIM_REG_PGU_X_RES_MASK;
	...
	case ARCPGUFB_GET_OL_END:
		tmplong =
		    (fb_devdata.regs->
		     ol_scn_line & DISP_DIM_REG_PGU_Y_RES_MASK) >>
		    DISP_DIM_Y_SHIFT;
		return copy_to_user((void *)arg, &tmplong,
				    sizeof(unsigned long)) ? -EFAULT : 0;
	...
	case ARCPGUFB_SET_OL_OFFSET:
		if (!copy_from_user
		    ((void *)&result, (void *)arg, sizeof(unsigned long))) {
			fb_devdata.regs->ol_frm_st =
			    fb_devdata.fb_virt_start + result;
			return 0;
		}
		return -EFAULT;
	...
	case ARCPGUFB_SET_STATCTRL:
		...
		down(&pgu_sem);
		result =
		    copy_from_user((void *)&(fb_devdata.regs->statctrl),
				   (void *)arg,
				   sizeof(unsigned long)) ? -EFAULT : 0;
		for (tmpint = 0; tmpint < 1000; tmpint++) ;
		up(&pgu_sem);
	...
	case ARCPGUFB_SET_VCLK:
		if (cpld_controls_pll) {
			...
			down(&pgu_sem);
			if (!copy_from_user
			    ((void *)&tmplong, (void *)arg,
			     sizeof(unsigned int))) {
				stop_arcpgu();
				set_vlck(tmplong);
				fb_devdata.regs->statctrl |=
				    STATCTRL_REG_DISP_EN_MASK;
			} else
				result = -EFAULT;
			up(&pgu_sem);
			return result;
	...
	case ARCPGUFB_GET_VCLK:
		if (cpld_controls_pll) {
			...
			return copy_to_user((void *)arg, (void *)&(tmplong),
...
static int arc_pgu_probe(struct platform_device *dev)
{
	...
	printk(KERN_INFO "ARC PGU: controller version %lu, using the %s\n",
	       ((fb_devdata.regs->
		 pgu_id & PGU_VERSION_MASK) >> PGU_VERSION_SHIFT),
	...
	/* This bit's easy */
	fb_devdata.rgb_size =
	    CONFIG_ARCPGU_RGBBUFS * (fb_devdata.display->xres *
	...
	fb_devdata.yuv_max_size =
	    CONFIG_ARCPGU_YUVBUFS * ((y_res * x_res) + ((y_res * x_res) / 2) +
				     (4 * DEFAULT_ALIGN_SIZE));
	 ...
	 */
	fb_devdata.fb_size =
	    fb_devdata.rgb_size + fb_devdata.yuv.yuv_size +
	    ((CONFIG_ARCPGU_YUVBUFS) * DEFAULT_ALIGN_SIZE);

	/* FIXME squelch buffer added for resolutions > VGA */
	fb_devdata.fb_size += 0x200;
	printk
	    ("ARC PGU: display/RGB resolution %03ldx%03ld, YUV resolution %03dx%03d\n",
	     fb_devdata.display->xres, fb_devdata.display->yres,
	     arcpgu_yuv_xres, arcpgu_yuv_yres);
	printk
	    ("Framebuffer: Size = 0x%x (RGB: 0x%lx, YUV: 0x%lx) %d RGB, %d YUV\n",
	     fb_devdata.fb_size, fb_devdata.rgb_size, fb_devdata.yuv.yuv_size,
	     CONFIG_ARCPGU_RGBBUFS, CONFIG_ARCPGU_YUVBUFS);
	...
	for (page = fb_devdata.fb_virt_start;
	     page < BUF_ALIGN(fb_devdata.fb_virt_start + fb_devdata.fb_size,
			      DEFAULT_ALIGN_SIZE); page += PAGE_SIZE) {
	...
	if (!
	    (fb_devdata.info.pseudo_palette =
	     kmalloc(sizeof(u32) * 16, GFP_KERNEL)))
		return -ENOMEM;
	...
	memcpy(&fb_devdata.default_yuv, &fb_devdata.yuv,
	       sizeof(struct yuv_info));
	...
	while ((this_opt = strsep(&options, ",")) != NULL) {
		if (!strncmp(this_opt, "display:", 6)) {
			...
			for (i = 0; i < num_displays; i++) {
				if (!strncmp
				    (this_opt + 6, displays[i].display_name,
				     strlen(this_opt))) {
					display_index = i;
					break;
				}
