			...
			int length, int *eof, void *data)
{
	...
	len +=
	    sprintf(buf + len, "display_q len = %d\n", kfifo_len(&display_q));
	...
	len +=
	    sprintf(buf + len, "frame_rate=%d/1000\n",
		    (int)(1000 * frame_rate));
...
static void refire_timer(void)
{
	...
	run_len++;
	flip_timer.expires =
	    (unsigned long)(run_start + run_len * exact_frame_delay);
...
int arc_vsync_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case ARCPGUFB_SEND:
		...
		if (timer_pending(&flip_timer) || is_paused)
			return 0;
		if (time_before_eq
		    (last_frame_timestamp + (int)frame_delay, jiffies)) {
