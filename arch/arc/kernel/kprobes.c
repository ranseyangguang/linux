...
static void __kprobes dsmArcInstr(unsigned long addr, struct arcDisState *state)
{
	...
	case op_BLC:
		...
		if (IS_BIT(state->words[0], 16)) {
			...
			fieldA =
			    (((BITS(state->words[0], 15, 15) << 7) |
			      (BITS(state->words[0], 17, 23))) << 1);
	...
	case op_MAJOR_4:
		...
		case 35:	/* "JLcc.D" */
			...
			if (!fieldCisReg) {
				state->target = fieldC;
				state->flow =
				    is_linked ? direct_call : direct_jump;
			} else {
				state->reg_jump = fieldC;
				state->flow =
				    is_linked ? indirect_call : indirect_jump;
			}
			state->is_branch = 1;
			break;
...
unsigned long __kprobes get_reg(struct pt_regs *regs, unsigned long reg_no)
{
	...
	case 31:
		return regs->blink;
	default:
		pr_info("register %ld not part of pt regs\n", reg_no);
		return 0;
	}
}
	...
	*fall_thru = pc + instr.instr_len;
	...
	if (instr.delay_slot) {
		...
		*fall_thru += instr_d.instr_len;
		...
		/*
		 * We cannot step into delay slot. If we put a trap in the
		 * delay slot, we cannot identify that it is from delay slot
		 * and cannot set DE bit when the actual instruction is
		 * painted and executed.
		 */
	}
...
static void __kprobes setup_singlestep(struct kprobe *p, struct pt_regs *regs)
{
	...
	*(p->addr) = p->opcode;
	...
	if (regs->status32 & 0x40) {
		...
		is_branch =
		    next_pc((unsigned long)p->addr, regs, &fall_thru, &target);
...
int __kprobes kprobe_handler(unsigned long addr, struct pt_regs *regs)
{
	...
	if (p) {
		 ...
		 * hit while within the handler, we save the original kprobes
		 * and single step on the instruction of the new probe without
		 * calling any user handlers to avoid recursive kprobes.
		 */
		if (kprobe_running()) {
