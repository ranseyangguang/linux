...
static int get_reg(unsigned int reg, struct pt_regs *regs,
		   struct callee_regs *cregs, long *val)
{
	long *p;
...
static int set_reg(unsigned int reg, long val, struct pt_regs *regs,
		     ...
		     unsigned long cause, struct callee_regs *cregs)
{
	...
	if (write == 0) {
		...
		if ((aa == 1) || (aa == 2)) {
			...
			if (zz == 2) {
				if (set_reg
				    (wb_reg, src2 + (src3 << 1), regs, cregs))
					goto bad;
			} else if (zz == 0) {
				if (set_reg
				    (wb_reg, src2 + (src3 << 2), regs, cregs))
					goto bad;
			} else
				goto bad;
		}
...
fault:
	pr_err("Alignment trap: fault in fix-up "
		"%0*lx at [<%08lx>]\n", inst_16 ? 4 : 8, instr, instrptr);
