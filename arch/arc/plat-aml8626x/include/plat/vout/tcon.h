...
#define MLVDS_DIGITAL           8       //miniLVDS Interface


#ifndef CONFIG_AM_LCD_MLVDS_OUTPUT

// For traditional RGB TTL interface Panel
typedef struct {
...
// CONFIG_AM_MLVDS_OUTPUT 

// In A3 chips, we support miniLVDS interface panels.
// And rewrite the tcon_conf_t.

// In A3 chips, there are 8 TCON channels for Dual Gate.
// TCON0~3 is exactly same FULL function and TCON4~7 is exactly same(only uses HALF function).
// Full function means that there are 8 registers to control the timing.
// First 4 regs are for First half tcon and Second 4 regs are for Second half tcon.
// Half tcon is defined as HS/HE/VS/VE(Horizon/Vertical/Start/End)
// HALF function means that there are only 4 registers to control the timing.
// The Second half tcon is delayed Half horizon as First half tcon.
// The Second half tcon is same as First half tcon.
typedef struct {
    int channel_num;
    int hv_sel;
    ...
    int tcon_2st_hs_addr;
    int tcon_2st_he_addr;
    int tcon_2st_vs_addr;
    int tcon_2st_ve_addr;
}mlvds_tcon_config_t;

// For MiniLVDS PHY control
typedef struct {
...
typedef struct {
    ...
    mlvds_config_t *mlvds_config;   //New added
...
#endif /* CONFIG_ARCH_AMLOGIC_APOLLO_A3 */
#endif /* TCON_H */
