...
/* Ioctl selectors
 * FIXME - make sure these are unique within the fb subsystem
 */
...
/* return address of yuv buffer n */
#define ARCPGUFB_GET_YUV_BASE	0xffff002c
...
/* YUV buffer layout - all sizes and offsets in bytes. */
...
/* Base+0x08    CLK_CFG_REG
 *	Clock Configuration register. Used to Define the clock divider value
 *	and the pulse width (high cycle) in clock cycles:
...
#define ENCODE_PGU_DIMS(x, y) ((x & DISP_DIM_REG_PGU_X_RES_MASK) | \
			      ((y & DISP_DIM_REG_PGU_X_RES_MASK) * 0x10000))

/*	Base+0x18    HSYNC_CFG_REG   Hsync Configuration register.
 *	This is used to specify various hsync related timings in clock cycles:
...
/*	Base+0x20    VSYNC_CFG_REG   Vsync Configuration register.
 *	This is used to specify various Vsync related timings in
 *	Hsync cycles:
...
/*	Base+0x28   FRMSTART_REG     Frame Start Address.
 *	This 32 bit address specifies the start of the display frame
...
/*	Base+0x38   FRM_DIM_REG	     Frame Dimension register.
 *	This register defines the frame size of the display in
 *	memory and can be smaller than the display.
 *
 *	[10:0] FRM_WTH  : frame line width in pixels.
 *	Must be <= PGU_X_RES without x2 mode
 *	or <= PGU_X_RES/2 in x2	mode.
 *
 *	[26:16] FRM_LNS : frame number of lines
 *	Must be <= PGU_X_RES
 *	without x2 mode or <= PGU_X_RES/2 in x2 mode.
 */
...
/*
 *	Base+0x40   STATCTRL_REG     Status and Control register.
 */
#define STATCTRL_REG_OFFSET 0x40

/*	[0] DISP_EN   : Display enable bit.
 *	When set to high, starts the display. It then clears after a few cycles
 *	if the CONT bit is not set, hence allowing the display to do the frame
 *	once.
 */
#define STATCTRL_REG_DISP_EN_MASK 1

/*	[1] INTR_EN   : Interrupt Enable. Active high. */
#define STATCTRL_INTR_EN_MASK 2

/*	[2] INTR_CLR  : Interrupt Clear.
 *	Set to high to clear the interrupt flag/request. Always reads as 0 */
#define STATCTRL_INTR_CLR_MASK 4

/*	[3] CONT	   : Continuous mode.
 *	When set, will not allow the DISP_EN bit to be cleared by itself and as
 *	long as DISP_EN is not cleared manually, the Dispaly will drive the PGU
 *	continuously, frame after frame.
 */
#define STATCTRL_CONT_B_MASK 8

/*	[5:4] PIX_FMT   Pixel Format. Select the pixel format:
 *	"00" - RGB555
 *	"01" - YUV420 (aka YV12)
 *	"10" - reserved
 *	"11" - reserved */
...
/*	[6] BACKLIGHT   Active high backlight enable.
 *	When set to high, sets the back light control signal to high to turn
 *	on the backlight of the PGU.
 */
#define STATCTRL_BACKLIGHT_MASK 0x40

/*	[7] X2 Zoom 2 mode.
 *	when set, every pixel in the frame is display twice in a line & the
 *	line is displayed twice therefore enlarging the picture.
 */
#define STATCTRL_X2_MASK 0x80

/*	[8] REV_LN_SCAN Reverse line scan.
 *	When set the display displays the last line first, back tow first
 */
...
typedef volatile struct {
	unsigned long pgu_id;		/* Identity register */
	unsigned long padding1;
	unsigned long clk_cfg;		/* Clock control */
	unsigned long padding2;
	unsigned long disp_dim;	/* Display dimension */
	unsigned long padding3;
	unsigned long hsync_cfg;	/* HSYNC */
	unsigned long padding4;
	unsigned long vsync_cfg;	/* VSYNC */
	unsigned long padding5;
	unsigned long frm_start;	/* Frame start */
	unsigned long padding6;
	unsigned long stride;		/* Frame stride */
	unsigned long padding7;
	unsigned long frm_dim;		/* Frame dimension */
	unsigned long padding8;
	unsigned long statctrl;	/* Status and control */
	unsigned long padding9;
	unsigned long cru_frm_st;	/* Chroma U frame start */
	unsigned long padding10;
	unsigned long cru_stride;	/* Chroma U stride */
	unsigned long padding11;
	unsigned long crv_frm_st;	/* Chroma V frame start */
	unsigned long padding12;
	unsigned long crv_stride;	/* Chroma V stride */

	unsigned long padding13;
	unsigned long ol_frm_st;	/* Overlay frame start */
	unsigned long padding14;
	unsigned long ol_stride;	/* Overlay stride */
	unsigned long padding15;
	unsigned long ol_scn_line;	/* Overlay start and end line numbers */
	unsigned long padding16;
	unsigned long ol_col_key;	/* Overlay transpy colour key RGB 555 */

	unsigned long padding17;
	unsigned long centreoffsets;	/* X, Y offsets for centering */
} arc_pgu;
